#!/usr/bin/env python
__author__ = 'Aitor Blanco (aitor.blancomiguez@unitn.it, modified by Yunjeong So (yunjeong.k.so@gmail.com)'
__version__ = '4.1.1'
__date__ = '28 Aug 2024'

# Modified to Account for B. longum Subspecies and Unclassified Percentage.

import os
import time
import argparse as ap
import sys
sys.path.append('/services/tools/metaphlan/4.1.1/lib/python3.12/site-packages/metaphlan/utils/')
try:
...
def get_gtdb_profile(mpa_profile, gtdb_profile, database):
    ...
    sgb2gtdb = dict()
    with open("path_to/db/metaphlan_db_202403/mpa_vJun23_CHOCOPhlAnSGB_202307_SGB2GTDB.tsv", 'r') as read_file:
        ...
        with open(mpa_profile, 'r') as rf:
            ...
            for line in rf:
                if line.startswith('#mpa_'):
                    ...
                    abundances['s']['UNCLASSIFIED'] = unclassified
                elif 't__SGB' in line:
                    line = line.strip().split('\t')
