...
abundance_long <- gene_abundance %>%
  ...
  filter(
    ...
    Figure_5 %in% target_groups$AX |
      Figure_5 %in% target_groups$GM |
      Figure_5 %in% target_groups$GA
  )
...
p_GM <- make_target_plot(wide_for_plot, "GM", names(GM_colors), ann_GM, ymax)
p_GA <- make_target_plot(wide_for_plot, "GA", names(GA_colors), ann_GA, ymax)
